...
const punctuation = (i, len) => {
  if (i == len - 2 && len > 2)
    return (
      <h6 className="inline margin-0" key={`punc-${i}`}>
        , and&nbsp;
      </h6>
    );
  if (i == len - 2 && len > 1)
...
const generateAuthors = authors => {
  ...
  if (len === 4)
    authorElements.push(<h6 className="inline bold"> {len - 3} more</h6>);
  else if (len > 4)
    authorElements.push(<h6 className="inline bold">and {len - 3} more</h6>);
...
export const PublicationListItem = ({ post }) => (
  ...
  >
    ...
    <div className="card__text">
      <a href={post.url}>
        {post.date && (
          <h6 className="card__text__date margin-top-0 margin-bottom-5 margin-bottom-md-15">
      ...
      {post.programs && (
        <h6 className="card__text__program caption margin-top-5 margin-top-md-15 margin-bottom-0">
